...
JOB 2 — INTENT LAYER, once per ICP (no web). Each icp_targeting entry carries its own intent_filters block; fit AND intent both required. The buying signals usually come from the brief and may be identical across entries — that is fine; tailor q_organization_job_titles to an ICP when a signal names roles specific to it.
