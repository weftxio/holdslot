...
| **GD-3** | Where does Stripe config live? | Secret envelope **`holdslot/prod/stripe`** `{api_key, webhook_signing_secret, price_launch, price_growth, price_activation, price_qualified_meeting, meter names}` (the smartlead-envelope precedent) + per-tenant ids on the `subscription` row. `price_qualified_meeting` is the metered $500/meeting **price id** (distinct from the `qualified_meeting` meter name) — L3: a paid subscription created without it bills $0/meeting silently, so it is in the seeding contract + `verify_keys.py --strict` gate. `verify_keys.py` gains `check_stripe`. |
