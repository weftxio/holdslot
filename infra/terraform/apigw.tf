# API Gateway HTTP API → Lambda alias (proxy). One catch-all route now; A4 adds auth at
# the application layer (the central guard), not here, so route config stays trivial.
